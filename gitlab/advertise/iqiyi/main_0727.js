    ...
    init: function() {
        ...
        return new Promise(function(resolve, reject) {
            iqiyi(function(info){
                ...
                var jq = new info.jq();
                self.jquery = jq.$.fn.constructor.expando;
                //console.log(self);
                resolve(self);
            });
        });
...
function showSendToApi(pstring, ua, refer) {
    console.log(pstring);
    var post_options = {
...
function show2(user, vinfo) {
    ...
    var eventid = md5(vinfo.tvId + "IQIYI" + (new Date()).getTime());
    show.addParam({
        adPlayerId: "qc_100001_100226",//??
        // adPlayerId: "qc_100001_100226",//??
        passportId: "",  //可不填?
        requestId: md5(eventid+Date.now()),//每个视频不一样??
        timestamp: Date.now(),
        // timestamp: 1501120149855,
        uaaUserId: user.uid,//自添加,每个用户相对固定
        // uaaUserId: "6bcaef4273db7d71eb8384bce4b6150b",//自添加,每个用户相对固定
        //startTime:1501120149800,
        //_:1501120149811
    });
    show.addEncryptionParam({
        ...
        encryptionVersion:"",
        episodeId:vinfo.tvId,          // videoQipuId || tvId
        isPreload:0,
        isVIP:0,
        needPolicy:1,
        passportCookie:"",
        ...
        url:"http://www.iqiyi.com/w_19rtwdhyn1.html",
...
var Flush = function() {
    ...
    var pstring = show2(user, vinfo);
    var pstr = pstring.slice(26);
    var _time = Date.now()
    pstr=pstr+"&cb="+user.jquery+"_"+_time+"&_="+(_time-1002);
    showSendToApi(pstr, user.ua, link);
